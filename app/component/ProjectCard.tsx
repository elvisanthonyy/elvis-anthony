  ...
  return (
    <div className="relative flex shadow-2xl/8  flex-col md:flex-row lg:flex-col aspect-[5/6] mb-10 mx-auto overflow-hidden w-[90%] lg:h-120  md:h-75 lg:w-full xl:flex-row xl:h-80 md:aspect-auto h-fit bg-white shadow-3xl rounded-2xl ">
      ...
      <div className="flex flex-col shrink-0 md:h-full lg:w-full lg:h-[50%] md:w-[50%] xl:w-[50%] xl:h-full h-[60%] p-3 w-full">
        <div className="text-xl  text-black m-3">{projectTitle}</div>
        <div className="ml-3 h-[60%] bg-amber-200lg:h-[45%] w-[90%] text-xs md:text-md text-gray-800">
          {projectDescription}
        </div>
        ...
        <div className="ml-3 flex items-center my-3  h-15 m-0 w-[80%] ">
          <a href={demoUrl} target="_blank">
            <div className="transition-all duration-300 ease-in flex px-4 justify-center items-center min-w-20  w-fit h-9 rounded-lg border-1 text-xs mr-2 text-white bg-black hover:opacity-50">
              {projectTitle === "Lyric Church" ? "Live" : "Live Demo"}
            </div>
          </a>

          <a href={codeUrl} target="_blank">
            <div className="transition-all duration-300 ease-in  flex px-2 justify-center items-center min-w-20  w-fit h-9 rounded-lg border-1 text-xs mr-2 text-white bg-black hover:opacity-50">
              Code
            </div>
          </a>
        </div>
