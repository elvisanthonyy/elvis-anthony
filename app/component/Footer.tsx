...
import { location } from "@/libs/database";
...
const Footer = ({ project, myContact }: ChildProps) => {
  ...
  return (
    <footer
      className={`flex py-6 lg:px-[20%] mt-10 px-5 pb-10 flex-col md:items-start text-start md:text-left md:px-15 md:pt-5 md:min-h-60 md:h-fit items-start justify-evenly ${
        theme === "light"
          ? "text-nav-gray bg-light-black"
          : "bg-lighter-black text-nav-gray"
      }  w-full min-h-100`}
    >
      <div className="text-xl font-bold text-left mb-3">Elvis Anthony</div>
      <div className="w-full gap-6 items-start flex flex-col mt-5  md:justify-between">
        ...
        <div className="h-30 md:h-30 flex item-start flex-col md:w-90 text-left justify-between ">
          <div
            onClick={() => {
              window.scrollTo({
                top: 0,
            ...
            className="transition-all duration-300 ease-in md:w-15 cursor-pointer text-sm hover:text-md hover:text-green-500"
          >
            Home
          </div>
          <div
        ...
        <div className="w-50 mt-10 md:h-full md:items-center md:mt-0 text-xl text-dark-white justify-between flex h-10">
          <a
            target="_blank"
          ...
          <a target="_blank" href="https://github.com/elvisanthonyy">
            <FiGithub className="cursor-pointer hover:text-green-500" />
          </a>
          <a
            target="_blank"
